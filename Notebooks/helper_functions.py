    ...
    for fold in os.listdir(raw_data_path):
        ...
        for cls in os.listdir(raw_data_path + "/" + fold):
            print(
                ...
                cls,
                "has : ",
                len(glob(raw_data_path + "/" + fold + "/" + cls + "/*")),
                "files",
            )
        print("-" * 35)
    return None
...
def visulize_multi_class_raw_images(raw_data_path: str, fold: str):
    ...
    col = 0
    for cls in os.listdir(raw_data_path + "/" + fold):
        sample_img = random.sample(
            glob(raw_data_path + "/" + fold + "/" + cls + "/*"), k=1
        )[0]
...
def prepare_dataset(ds, data_augmentation, resize_and_rescale, augment=False):
    """
...
def visulize_batch_from_dataset(ds, classes_list):
    ...
    for x, y in ds:
        for batch in range(0, 32):
            try:
                axs[row][col].imshow(x[batch] / 255.0)
...
def plot_model_hist(hist_var):
    ...
    pd.DataFrame(hist_var.history)[["loss", "val_loss"]].plot(
        title="Loss Plots", ax=axes[0], figsize=(10, 5)
    )
    pd.DataFrame(hist_var.history)[["accuracy", "val_accuracy"]].plot(
        title="Accuracy Plots", ax=axes[1], figsize=(10, 5)
    )
    return None
